...
import { formatPrice, prazoLabel, isSaoCarlos } from "@/data/products";
  ...
  if (items.length === 0 && !finalizado) {
    return (
      <main className="mx-auto flex min-h-[60vh] w-full max-w-4xl flex-col items-center justify-center px-4 py-12 text-center">
        ...
        <Link href="/" className="mt-4 rounded-full bg-black px-6 py-3 text-sm font-semibold text-white">
          Ver produtos
        </Link>
      </main>
    );
    ...
    return (
      <main className="mx-auto flex min-h-[60vh] w-full max-w-2xl flex-col items-center justify-center px-4 py-12 text-center">
        <div className="rounded-2xl border bg-emerald-50 p-8">
          ...
          <p className="mt-4 text-xl font-bold text-emerald-800">Pedido confirmado!</p>
          ...
          <Link href="/" className="mt-6 inline-block rounded-full bg-black px-6 py-3 text-sm font-semibold text-white">
            Voltar à loja
          </Link>
        </div>
      </main>
            ...
            <ul className="mt-3 divide-y text-sm">
              {items.map(({ product, quantity }) => (
                <li key={product.id} className="flex justify-between py-2">
                  ...
                  <span className="font-bold">{formatPrice(product.preco * quantity)}</span>
                </li>
              ))}
            </ul>
          </section>
              ...
              {freteValor !== null && (
                <div className="text-sm">
                  <span className="text-emerald-700">
                    Frete: {freteValor === 0 ? "Grátis (São Carlos!)" : formatPrice(freteValor)}
                  </span>
          ...
          <section className="rounded-xl border p-4">
            ...
            <div className="mt-3 flex flex-col gap-2">
              <label className="flex items-center gap-3 rounded-lg border p-3 cursor-pointer has-[:checked]:border-black has-[:checked]:bg-zinc-50">
                <input type="radio" name="pagamento" value="pix" checked={metodo === "pix"} onChange={() => setMetodo("pix")} />
                <div>
              ...
              <label className="flex items-center gap-3 rounded-lg border p-3 cursor-pointer has-[:checked]:border-black has-[:checked]:bg-zinc-50">
                <input type="radio" name="pagamento" value="card" checked={metodo === "card"} onChange={() => setMetodo("card")} />
                <div>
              ...
              {isLocal && (
                <label className="flex items-center gap-3 rounded-lg border p-3 cursor-pointer has-[:checked]:border-black has-[:checked]:bg-zinc-50">
                  <input type="radio" name="pagamento" value="entrega" checked={metodo === "entrega"} onChange={() => setMetodo("entrega")} />
                  <div>
                    <p className="text-sm font-medium">Pagar na Entrega</p>
                    <p className="text-xs text-zinc-500">Cartão ou PIX no recebimento · São Carlos</p>
                  </div>
                </label>
              )}
            </div>
          ...
          <div className="sticky top-20 rounded-xl border p-4">
            ...
            <div className="mt-3 flex flex-col gap-2 text-sm">
              ...
              <div className="flex justify-between">
                <span>Frete</span>
                <span>
                  {freteValor === null ? "—" : freteValor === 0 ? "Grátis" : formatPrice(freteValor)}
                </span>
              </div>
              <hr />
